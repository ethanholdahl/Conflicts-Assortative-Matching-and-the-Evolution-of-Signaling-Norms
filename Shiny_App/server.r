...
function(input, output, session) {
  ...
  output$PA = renderPlot({
    ggplot(data = pop_evo(input$ratio, input$ratio_h, input$ratio_l, input$vHH, input$vHL, input$vLH, input$vLL, input$A, input$time), aes(x = t, y = Proportion, color = Type)) +
      geom_line()
  })
}

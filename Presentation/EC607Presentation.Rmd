...
date: EC 607 Presentation (`r format(Sys.time(), '%d %B %Y')`)
output:
...
- Costly signaling is honest (Handicap Principle Zahavi 1975)
...
- Costly signling can be seen in some but not all animals, what determines where we see this signaling?
...
We theorize that the role of costly signaling in between group competition is that it increases the speed of evolution in the signaling population. As a result, if two otherwise identical populations come into contact with each other the population that wins out depends on a few key paramater: the amount of time before competiton, the cost of the signal, and the intensity of competition.
This model might explain why some classes of life and biomes are more likely to exhibit costly signaling. For example, birds and fish with their mobility likely have shorter periods of seperation between sister species compared to other animals. Areas like rainforests that have firece competition for resources in every niche may also exhibit a higher proportion of signalers since losing populations would struggle to find an available ecological niche to be displaced to. 